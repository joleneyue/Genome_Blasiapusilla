## 1. Genome annotation - TE annotation 
...
Give the LTR_retriever path to the EDTA.pl
...
Run the same for the genome first 9 scaffolds
...
Copy Marchanthia file to the server 
...
Re-run EDTA for the blasia female and male mixure:
...
Do this for Blasia female genome
...
Do the same for Marchanthia genome:
...
Do the same for the female genome:
...
Now start to run the halstats and get the percentage identical of polca genome -9 scaffolds with feywei genome -9 scaffolds:
...
2.3. convert hal alignment to maf using the perl script from augustus:
...
2.4. Loading genomes into SQLite database
...
2.5. Load RNA-Seq Hints into SQLite DB
...
2.7. Run comparative Augustus with RNAseq hints from Zurich
...
## 3. comparative annotation on coge
...
## 4. genome assembly 's transcriptome assembly using stringtie and evaluation 
...
## 5. De novo_ Repeat Identification: softmask the genome for braker
